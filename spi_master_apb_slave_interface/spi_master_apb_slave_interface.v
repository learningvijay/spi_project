module spb_master_apb_slave(
    input p_clk,p_reset,p_select,p_enable,p_write,p_ready,p_slverr,spi_interrupt_request
    input [2:0]p_addr,
    input [7:0]p_data_in,
    input ss,
    input receive_data,
    ...
    output [7:0]data_to_send_buffer_reg,
    output spi_mode,
    output spr,sppr
);
endmodule
...
assign wire spie = spicr1[7];
assign wire spe = spicr1[6];
assign wire sptie = spicr1[5];
assign mstr = spicr1[4];
assign cpol = spicr1[3];
assign cpha = spicr1[2];
assign wire ssoe =spicr1[1];
assign lsbfe = spicr1[0];
assign wire modfen =spicr2[4];
assign wire bidiroe[3];
assign spiswai = spicr2[1];
assign wire spco = spicr2[0];
...
always @(posedge p_clk or negedge p_reset) begin
    ...
    else if ((spidr==p_data_in)&&(spidr !=data_to_send_buffer_reg)&&(spi_mode==spi_run)&&(spi_mode==spi_wait)) begin
        send_data<=1'b1;
...
always@(*) begin
    case (PRESENT_STATE)
    begin
    IDEL:if(!p_enable && p_select) begin
                NEXT_STATE=SETUP;
            end
            else begin
                NEXT_STATE=IDEL;
